...
namespace TestBangazonAPI
{
    ...
    public class ProductTest
    {
        ...
        public async Task<Product> createMug(HttpClient client)
        {
            Product redMug = new Product
            {
                ProductTypeId = 1,
                CustomerId = 1,
                Price = 5000,
                Title = "Red Mug",
                Description = "This mug is very red",
                Quantity = 5
            };
            string redMugAsJSON = JsonConvert.SerializeObject(redMug);


            HttpResponseMessage response = await client.PostAsync(
                "api/Product",
                new StringContent(redMugAsJSON, Encoding.UTF8, "application/json")
            );

            response.EnsureSuccessStatusCode();

            string responseBody = await response.Content.ReadAsStringAsync();
            Product newRedMug = JsonConvert.DeserializeObject<Product>(responseBody);
        ...
        public async Task deleteRedMug(Product redMug, HttpClient client)
        {
            HttpResponseMessage deleteResponse = await client.DeleteAsync($"api/Product/{redMug.Id}?delete=True");
